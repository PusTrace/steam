...
class EVA(BaseStrategy):
    ...
    def dinamyc_thresholds(self):
        ...
        print(self.skin_orders[0])
        matching_order = next(order for order in self.skin_orders[0] if order[0] < default_threshold_top)
        if matching_order:
            threshold_top = matching_order[0]
            print(f"[EVA] Matching order found: {matching_order}")
            if matching_order[1] < self.volume * 1.5:
                return threshold_top
            else:
                print("[EVA] Matching order found, but volume is too much, skipping")
                return None
    ...
    def analyze_order_book(self):
        ...
        # === Поиск ордеров до верхней границы ===
        if threshold_top is None:
            print("[EVA] threshold_top is None")
        orders_in_range = [o for o in self.skin_orders[0] if o[0] <= threshold_top]
        ...
        if walls:
            print(f"[EVA] Найдено {len(walls)} стенок в диапазоне")
            print(f"[EVA] Стенки: {walls}")
            ...
            y = min_price
            print("[EVA] Стенок не найдено — используем минимальную цену в диапазоне")

        snapshot = self.do_snapshot(y=y, threshold_top=threshold_top)
    ...
    def do_snapshot(self, **kwargs):
        ...
        volume_before_y = next((order[1] for order in self.skin_orders[0] if order[0] < y), None)
        volume_before_threshold_top = next((order[1] for order in self.skin_orders[0] if order[0] < threshold_top), None)
        return {"volume_before_y": volume_before_y,
...
def test_eva():
    ...
    y, amount, snapshot = pt_model.decide(skin_orders=skin_orders, slope_six_m = -6.936237812244004, slope_one_m = -8.047897084084791, avg_month_price = 2186.0670585952857, avg_week_price = 2165.141007828283, volume = 170, high_approx = 2378.7958994708993, low_approx = 1937.7979948717948, moment_price = 2011.404)
