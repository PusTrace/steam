...
for skin, skin_info in perfect_data.items():
    ...
    target_price_40 = median_price * 0.4 if median_price is not None else lowest_price * 0.4
    ...
    for i in range(len(buy_order_graph) - 1):
        ...
        if price_high > target_price_87 > price_low:
            # Проверяем, что ключа "order_info" ещё нет
        ...
        if price_high > target_price_40 > price_low:
            # Проверяем, что ключа "order_info" ещё нет
            skin_info["order_info_40"] = buy_order_graph[i+1][1]
