...
def plot_analysis(
    raw_history: List[obj.ItemPriceHistory],
    cleaned_history: List[obj.ItemPriceHistory],
    slope_6m: float,
    intercept_6m: float,
    slope_1m: float,
    intercept_1m: float,
    ...
    fig.suptitle(
        "Price History Analysis (Last 12 Months)", fontsize=16, fontweight="bold"
    )
    ...
    raw_filtered = [
        (item.date, item.price) for item in raw_history if item.date >= one_year_ago
    ]
    raw_dates = [r[0] for r in raw_filtered]
    raw_prices = [r[1] for r in raw_filtered]

    cleaned_filtered = [
        (item.date, item.price) for item in cleaned_history if item.date >= one_year_ago
    ]
    ...
    ax.scatter(
        ...
        marker="o",
        zorder=5,
    )  # zorder — порядок слоёв (больше = выше)
    ...
    if slope_6m_data:
        ...
        filtered_6m = [
            (d, p)
            for d, p in zip(regression_dates_6m, regression_prices_6m)
            if d >= one_year_ago
        ]
        if filtered_6m:
    ...
    ax.set_title(
        "Price Trends & Outlier Removal", fontsize=13, fontweight="bold", pad=15
    )
